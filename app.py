...
def create_sensor_data_table(table_name):
    class LevelSensorData(db.Model):
        ...
        __table_args__ = {'extend_existing': True}  # Allow extension of existing table
        id = db.Column(db.Integer, primary_key=True)
        date = db.Column(db.DateTime)
    ...
    # Use extend_existing=True to redefine the existing table
...
def level_sensor_data(admin_name):
    ...
    if request.method == 'POST':
        ...
        data = LevelSensorData.query.all()
        return jsonify([{
            'id': d.id,
            'date': d.date,
...
@app.route('/api/admin/<adminname>/sensor_data', methods=['POST'])
def add_sensor_data(adminname):
    try:
        ...
        if table_name not in inspector.get_table_names():
            create_table_for_admin(adminname)
